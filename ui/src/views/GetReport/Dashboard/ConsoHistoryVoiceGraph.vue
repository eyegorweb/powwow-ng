...
import {
  resumeFormattedValueFromSeconds,
  resumeAndTruncateFormattedValueFromSeconds,
} from '@/api/utils';
...
export default {
  ...
  methods: {
    async refreshData() {
      ...
      const voiceConsumption = data.voiceConsumption;

      const formattedData = voiceConsumption.reduce(
        (all, c) => {
      ...
      this.chartOptions = {
        ...
        yAxis: {
          labels: {
            formatter() {
              return resumeAndTruncateFormattedValueFromSeconds(this.value);
            },
            style: {
