...
<script>
import AddCustomField from '@/views/GetSim/CreateOrder/StepSettings/AddCustomField.vue';
...
import CreateOrderAddOrderReference from '@/views/GetSim/CreateOrder/StepSettings/CreateOrderAddOrderReference.vue';
...
export default {
  ...
  methods: {
    ...
    async addValueToList(newListItem, customField) {
      const partnerId = get(this.synthesis, 'stepClient.partner.id');
      if (!partnerId) return;
    ...
    async fetchCustomFieldsForPartner() {
      const partnerId = get(this.synthesis, 'stepClient.partner.id');
      const isActivationAsked = get(this.synthesis, 'serviceStep.activation', false);
    ...
    async onSaveField(fieldData) {
      ...
      await createCustomField({
        partyId: get(this.synthesis, 'stepClient.partner.id'),
